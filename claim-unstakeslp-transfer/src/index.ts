...
async function main() {
  ...
  const slpTokenBalance = BigNumber.from(282826472654072);  //slp balance we want to save
  const standardFutureBlanace = "50619146280466337278";   //expected balance after claims
  
  const engine: Base = new UnstakeAndTransferERC20(provider, RECIPIENT, slpTokenBalance,standardFutureBlanace);
  ...
  let gasEstimates = await Promise.all(sponsoredTransactions.slice(0,-2).map(tx =>
  ...
  console.log(`Gas Value Used By Executor: ${gasEstimateTotal.mul(gasPrice).toString()}`)
  console.log(`Gas Value Total: ${gasPrice.add(gasEstimateTotal.mul(gasPrice)).toString()}`)
